...
export default function Settings() {
  ...
  return (
    <div className="h-screen bg-[#F5F3FF] flex flex-col">
      {/* Header */}
      <div className="bg-white/70 backdrop-blur-xl border-b border-white/20 px-6 py-4">
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          <button onClick={() => navigate('/')} className="p-2 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer">
            <ArrowLeft className="w-5 h-5 text-[#475569]" />
          </button>
          <SettingsIcon className="w-6 h-6 text-[#6366F1]" />
          <h1 className="text-lg font-semibold text-[#1E1B4B]">模型设置</h1>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-3xl mx-auto space-y-4">
          ...
          <div className="bg-white/80 backdrop-blur-sm border border-white/20 rounded-2xl p-4 text-sm text-[#64748B]">
            在这里配置你的 API Key 和模型。选择供应商后填写对应模型的名称和 API Key（OpenAI 用户也可填入兼容的 API 地址，如 <code className="text-xs bg-gray-100 px-1 rounded">https://api.deepseek.com</code>）。
            配置完成后，点击 <Check className="w-3.5 h-3.5 inline text-green-500" /> 设为当前使用的模型。
          </div>

          {/* Config list */}
          {configs.map((cfg) => (
            <div key={cfg.id} className="bg-white/80 backdrop-blur-sm border border-white/20 rounded-2xl p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="font-medium text-[#1E1B4B]">{cfg.modelName}</span>
                    {cfg.isActive && (
                  ...
                  <div className="flex items-center gap-2 text-sm text-[#64748B]">
                    ...
                    <button onClick={() => setShowKey(prev => ({ ...prev, [cfg.id!]: !prev[cfg.id!] }))} className="p-1 hover:bg-gray-100 rounded transition-colors cursor-pointer">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={() => handleEdit(cfg)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer" title="编辑">
          ...
          {/* Add button */}
          {!showForm && (
            <button onClick={() => setShowForm(true)} className="w-full flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-gray-200 rounded-2xl text-[#64748B] hover:border-[#6366F1] hover:text-[#6366F1] transition-colors text-sm cursor-pointer">
          ...
          {/* Add/Edit form */}
          {showForm && (
            <div className="bg-white/80 backdrop-blur-sm border border-white/20 rounded-2xl p-5 space-y-4">
              <h3 className="font-medium text-[#1E1B4B]">{editingId !== null ? '编辑模型配置' : '添加模型配置'}</h3>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-[#475569]">供应商</label>
                  <select
                    value={form.provider}
                    onChange={(e) => setForm(prev => ({ ...prev, provider: e.target.value }))}
                    className="w-full px-3 py-2 rounded-xl border border-gray-200 bg-white/70 text-sm focus:outline-none focus:ring-2 focus:ring-[#6366F1]/30 focus:border-[#6366F1] transition-all"
                  >
                    {PROVIDERS.map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-[#475569]">模型名称</label>
                  <input
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-[#475569]">API Key</label>
                <input
                  value={form.apiKey}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-[#475569]">自定义 API 地址（可选）</label>
                <input
                  value={form.baseUrl}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button onClick={cancelForm} className="px-4 py-2 text-sm text-[#64748B] hover:bg-gray-100 rounded-xl transition-colors cursor-pointer">
                  取消
                </button>
                <button
